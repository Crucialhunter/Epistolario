  ...
  return (
    <ArchiveShell>
      <div className="mx-auto grid max-w-7xl gap-8 px-6 py-8 md:px-10 md:py-10 lg:px-16">
        <section className="grid gap-6 rounded-[2rem] border border-[#d8c89e] bg-[#2d2a26] px-6 py-8 text-[#fcfbf8] shadow-2xl md:px-8 lg:grid-cols-[minmax(0,1.25fr)_360px] lg:px-10 lg:py-10">
          <div>
            <p className="text-[11px] font-bold uppercase tracking-[0.28em] text-[#d8be66]">Archivo epistolar navegable</p>
            <h1 className="reader-display mt-4 text-4xl font-semibold leading-tight text-white md:text-6xl">{overview.title}</h1>
            <p className="mt-5 max-w-3xl text-base leading-relaxed text-[#ebe3d1]">{overview.intro}</p>
            <div className="mt-6 flex flex-wrap gap-3 text-[11px] font-bold uppercase tracking-[0.22em] text-[#f0dfb4]">
              <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2">41 legajos</span>
              <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2">6701 cartas navegables</span>
              <span className="rounded-full border border-white/10 bg-white/5 px-4 py-2">Capa visual activa en 06, 10 y 19</span>
            </div>
            <div className="mt-8 flex flex-wrap gap-3">
              ...
              <Link href="/legajos/10" className="rounded-full border border-white/15 bg-white/5 px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.24em] text-[#fcfbf8] transition-colors hover:border-white/30 hover:bg-white/10">
                Entrar por legajo 10
              </Link>
            </div>
          </div>
          <div className="grid gap-4 rounded-[1.5rem] border border-white/10 bg-[#1b180d]/50 p-6">
            <div>
              <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#8f7742]">Como leer esta demo</p>
            ...
            <div className="rounded-[1.25rem] border border-white/10 bg-black/10 px-4 py-4 text-sm text-[#ebe3d1]">
              <p className="text-[11px] font-bold uppercase tracking-[0.22em] text-[#d8be66]">Ruta recomendada</p>
        ...
        <section className="grid gap-5">
          ...
          <div className="grid gap-5 md:grid-cols-2 xl:grid-cols-4">
            {overview.discoveryBlocks.map((block) => (
              <article key={block.id} className="rounded-[1.6rem] border border-[#e7dcc6] bg-white px-5 py-5 shadow-sm">
                <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#8f7742]">{block.eyebrow}</p>
                ...
                <div className="mt-4 flex flex-wrap gap-2">
                  {block.items.map((item) => (
                    <span key={item} className="rounded-full bg-[#f7f1e6] px-3 py-1 text-[10px] font-bold uppercase tracking-[0.18em] text-[#6a5d47]">{item}</span>
                  ))}
                </div>
              </article>
            ))}
