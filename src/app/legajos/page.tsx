...
export default async function LegajosPage() {
  ...
  return (
    <ArchiveShell headerProps={{ activeSection: 'legajos', contextMode: 'none' }}>
      <div className="mx-auto grid max-w-[1440px] gap-6 px-6 py-6 md:px-10 md:py-8 lg:px-16 lg:py-10">
        <section className="app-surface grid gap-5 rounded-[1.85rem] px-6 py-6 lg:grid-cols-[minmax(0,1.12fr)_340px] lg:px-8">
          <div>
            <SectionHeader
              eyebrow="Catalogo"
          ...
          <div className="app-surface-muted rounded-[1.4rem] p-5 text-sm text-[#6a5d47]">
            <p className="app-label">Lectura rápida</p>
        ...
        <section className="grid gap-5 lg:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_minmax(0,1.2fr)]">
          <article className="app-surface rounded-[1.55rem] px-5 py-5">
            <p className="app-label">Capa visual</p>
            ...
            <div className="mt-4 flex flex-wrap gap-2">
              {visualLegajos.map((legajo) => (
                <span key={legajo.id} className="app-chip rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em]">
                  Legajo {legajo.id}
                </span>
              ))}
            </div>
          </article>
          <article className="app-surface-muted rounded-[1.55rem] px-5 py-5">
            <p className="app-label">Escala</p>
          ...
          <article className="app-surface-muted rounded-[1.55rem] px-5 py-5">
            <p className="app-label">Sugerencia de uso</p>
