  ...
  return (
    <div className="mb-6 bg-white border border-ink/10 rounded-lg overflow-hidden">
      <div className="bg-stone/20 px-4 py-2 border-b border-ink/10 flex justify-between items-center">
        <h3 className="text-xs font-semibold text-ink/60 uppercase tracking-wider">Metadata Extraction</h3>
      </div>
        ...
        <table className="w-full text-sm text-left">
          <thead className="text-xs text-ink/60 bg-stone/5 border-b border-ink/5">
            <tr>
              ...
              <th className="px-4 py-2 font-medium">Model Prediction</th>
          ...
          <tbody className="divide-y divide-ink/5">
            {keys.map(key => {
              ...
              if (isMatch && !isMissingGt) {
                ...
                statusIcon = <span title="Found by model, missing in GT"><AlertCircle className="w-4 h-4 text-ink/40" /></span>;
                rowClass = "bg-stone/10";
              } else if (!isMissingGt && isMissingPred) {
                statusIcon = <span title="Missed by model"><X className="w-4 h-4 text-burgundy/50" /></span>;
                rowClass = "bg-burgundy/5";
              }
              ...
              return (
                <tr key={key} className={`hover:bg-stone/5 transition-colors ${rowClass}`}>
                  ...
                  <td className="px-4 py-2 text-ink/90">{gtVal !== undefined ? String(gtVal) : <span className="text-ink/30 italic">Not provided</span>}</td>
                  <td className="px-4 py-2 text-ink/90">{predVal !== undefined ? String(predVal) : <span className="text-ink/30 italic">Not extracted</span>}</td>
