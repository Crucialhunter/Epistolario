...
import { Search, MapPin, Calendar, User, ImageIcon, LayoutGrid, List as ListIcon, Star, X } from 'lucide-react';
...
export default function LetterExplorer({ legajoId, initialLetters, featuredIds = [] }: LetterExplorerProps) {
  ...
  const isFiltering = Boolean(search || filterLugar || filterRemitente || filterTema);
  const activeFiltersCount = [search, filterLugar, filterRemitente, filterTema].filter(Boolean).length;
  ...
  const LetterCard = ({ letter, isFeatured = false }: { letter: CartaSummary; isFeatured?: boolean }) => {
    if (effectiveViewMode === 'list' && !isFeatured) {
      const visibleThemes = letter.temas
        ? letter.temas.split(',').map((tema) => tema.trim()).filter(Boolean).slice(0, isMobile ? 1 : 2)
        : [];

      return (
        <Link
          href={`/legajos/${legajoId}/cartas/${letter.id_carta}?from=explorer`}
          className="group flex flex-col md:flex-row md:items-center justify-between bg-white border-b border-[#e7e1cf] px-3 py-3 md:px-6 md:py-5 hover:bg-[#fcfbf8] transition-colors gap-2 md:gap-4"
        >
          <div className="flex flex-col md:flex-row md:items-center gap-1.5 md:gap-6 min-w-0 flex-1 w-full">
            <div className="flex items-center justify-between md:shrink-0 md:w-24 mb-1 md:mb-0">
              <span className="text-base md:text-lg font-bold font-serif text-[#c5a028] group-hover:text-[#a38420] transition-colors">
                Carta {letter.id_carta}
              </span>
              <div className="md:hidden flex items-center gap-3">
                {letter.hasImages && <span className="text-[#5a5545]"><ImageIcon size={13} /></span>}
                <span className="text-[#c5a028] font-serif text-base leading-none">?</span>
              </div>
            </div>

            <div className="min-w-0 flex-1 flex flex-col md:grid md:grid-cols-12 gap-1.5 md:gap-4 md:items-center">
              <div className="md:col-span-4">
                <span className="font-semibold text-[#1b180d] text-sm md:text-base block truncate">{letter.remitente || 'Desconocido'}</span>
                <span className="text-[#5a5545] text-[11px] md:text-xs truncate block">a {letter.destinatario || 'Desconocido'}</span>
              </div>
              <div className="md:col-span-3 text-[11px] md:text-sm text-[#5a5545] flex items-center gap-1.5 md:block">
                <Calendar size={12} className="md:hidden shrink-0 text-[#c5a028]" />
                <span className="truncate">{letter.fecha || 'Sin fecha'}</span>
              </div>
              <div className="md:col-span-2 text-[11px] md:text-sm text-[#5a5545] font-medium flex items-center gap-1.5 md:block">
                <MapPin size={12} className="md:hidden shrink-0 text-[#c5a028]" />
                <span className="truncate">{letter.lugar || '---'}</span>
              </div>
              <div className="md:col-span-3 mt-1 md:mt-0 flex gap-1 overflow-x-auto no-scrollbar">
                {visibleThemes.map((tema) => (
                  <span key={tema} className="inline-flex items-center px-1.5 py-0.5 rounded bg-[#f3f0e7] text-[#5a5545] text-[9px] md:text-[10px] font-bold uppercase tracking-wider truncate max-w-[120px] whitespace-nowrap">
                    {tema}
                  </span>
                ))}
              </div>
  ...
  return (
    <div id="explorer-section">
      <div className="bg-white border border-[#e7e1cf] p-3 md:p-4 rounded-xl shadow-sm mb-8 md:mb-12 sticky top-3 md:top-4 z-40">
        <div className="flex flex-col gap-3">
          <div className="flex items-start md:items-center justify-between gap-3">
            <div className="min-w-0">
              <p className="text-[11px] font-bold uppercase tracking-[0.24em] text-[#a38420]">Explorador</p>
              <div className="flex items-center gap-2 flex-wrap">
                <p className="text-sm md:text-base font-semibold text-[#1b180d]">{filteredLetters.length} cartas visibles</p>
                {isFiltering && (
                  <span className="inline-flex items-center rounded-full bg-[#f3f0e7] px-2 py-0.5 text-[10px] font-bold uppercase tracking-[0.2em] text-[#5a5545]">
            ...
            <div className="hidden md:flex items-center bg-[#fcfbf8] border border-[#e7e1cf] rounded-lg p-1">
              ...
              <button
                ...
                title="Vista Mosaico"
                aria-label="Cambiar a vista mosaico"
              >
                <LayoutGrid size={18} />
              </button>
            </div>
          ...
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:gap-3">
            <div className="flex-1 relative">
              <Search size={17} className="absolute left-3 top-1/2 -translate-y-1/2 text-[#5a5545]" />
              <input
                type="text"
                placeholder="Buscar carta, remitente o lugar"
                value={search}
                onChange={(e) => setSearch(e.target.value)}
                className="w-full pl-10 pr-4 py-2.5 bg-[#fcfbf8] border border-[#e7e1cf] rounded-lg focus:outline-none focus:border-[#c5a028] focus:ring-1 focus:ring-[#c5a028] transition-all text-[#1b180d] text-sm"
              />
            </div>
            ...
            {isFiltering && (
              <button
                ...
                className="inline-flex shrink-0 items-center justify-center gap-2 bg-[#fcfbf8] border border-[#e7e1cf] text-[#c5a028] px-3 py-2.5 rounded-lg text-[11px] font-bold uppercase tracking-[0.2em] hover:bg-[#f3f0e7] transition-colors"
          ...
          <div className="grid grid-cols-2 gap-2 md:flex md:flex-wrap md:gap-2">
            <select value={filterLugar} onChange={(e) => setFilterLugar(e.target.value)} className="bg-[#fcfbf8] border border-[#e7e1cf] rounded-lg px-3 py-2 text-[#2d2a26] text-sm focus:outline-none focus:border-[#c5a028] min-w-0 md:min-w-[120px]">
              <option value="">Lugar (Todos)</option>
              {lugares.map((l) => <option key={l} value={l}>{l}</option>)}
            </select>

            <select value={filterRemitente} onChange={(e) => setFilterRemitente(e.target.value)} className="bg-[#fcfbf8] border border-[#e7e1cf] rounded-lg px-3 py-2 text-[#2d2a26] text-sm focus:outline-none focus:border-[#c5a028] min-w-0 md:max-w-[180px]">
              <option value="">Remitente (Todos)</option>
              {remitentes.map((r) => <option key={r} value={r}>{r}</option>)}
            </select>

            <select value={filterTema} onChange={(e) => setFilterTema(e.target.value)} className="col-span-2 bg-[#fcfbf8] border border-[#e7e1cf] rounded-lg px-3 py-2 text-[#2d2a26] text-sm focus:outline-none focus:border-[#c5a028] min-w-0 md:min-w-[140px]">
      ...
      {filteredLetters.length === 0 ? (
        <div className="text-center p-16 bg-white border border-dashed border-[#e7e1cf] rounded-xl">
          ...
          <button onClick={clearFilters} className="px-4 py-2 bg-[#f3f0e7] rounded-lg text-sm font-bold text-[#c5a028] hover:bg-[#e7e1cf] transition-colors">
            Limpiar filtros
          </button>
        </div>
      ) : (
