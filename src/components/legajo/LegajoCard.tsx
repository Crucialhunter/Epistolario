...
export default function LegajoCard({ legajo }: { legajo: LegajoCorpusVM }) {
  ...
  return (
    <Link
      ...
      className={`group flex h-full flex-col rounded-[1.7rem] border p-6 shadow-[0_18px_34px_rgba(44,44,42,0.07)] transition-all hover:-translate-y-0.5 hover:shadow-[0_24px_46px_rgba(44,44,42,0.12)] ${isVisual ? 'border-[#d8c89e] bg-[linear-gradient(180deg,#fbf7ef_0%,#f4ecd9_100%)] hover:border-[#c5a028]' : 'border-[#e7dcc6] bg-[linear-gradient(180deg,#fffdf9_0%,#f7f1e6_100%)] hover:border-[#c5a028]'}`}
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="app-label text-[#a38420]">Legajo {legajo.id}</p>
        ...
        <div className="grid justify-items-end gap-2 text-right">
          <span className="app-chip rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em]">
      ...
      {legajo.principalActors.length > 0 ? (
        <div className="mt-5 rounded-[1.1rem] border border-[#efe5d3] bg-[#fcfbf8]/86 px-4 py-4">
          <p className="app-label">Actores principales</p>
          <p className="mt-2 text-sm leading-relaxed text-[#6a5d47]">{legajo.principalActors.slice(0, 3).join(' · ')}</p>
        </div>
      ) : null}

      <div className="mt-5 grid gap-3 rounded-[1.1rem] border border-[#efe5d3] bg-[#fcfbf8]/86 px-4 py-4 sm:grid-cols-3">
        <div>
          <p className="app-label">Rango</p>
          <p className="mt-2 text-sm leading-relaxed text-[#6a5d47]">{legajo.dateRange}</p>
        </div>
        <div>
          <p className="app-label">Cartas</p>
          <p className="mt-2 text-sm leading-relaxed text-[#6a5d47]">{legajo.letterCount}</p>
        </div>
        <div>
          <p className="app-label">Modo</p>
      ...
      <div className="mt-5 flex flex-wrap gap-2">
        {legajo.keyPlaces.slice(0, 2).map((place) => (
          <span key={place} className="app-chip rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em]">{place}</span>
        ))}
        {legajo.keyThemes.slice(0, 2).map((theme) => (
          <span key={theme} className="app-chip rounded-full px-3 py-1 text-[10px] font-bold uppercase tracking-[0.2em]">{theme}</span>
        ))}
