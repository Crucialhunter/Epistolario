...
const literal1 = "1. He recivido la de V.M de 28 del passado 2. el Sr. Don Juan ha sido oy de purga, y leha 3. hido muy bien con ella se le conoze esta 4. muy mejorado, los medicos estan atentos y leh desengañado que si no llena 5. la regla que le dexaran asi en el bever como 6. en lo demas que perezera, con que ba en el. 7. El Sr. Don Po esta bueno, hasta agora 8. poco se ha andado en esta semana 9. se comenzara, de lo que fuere sucediendo avisose 10. la carta dara V.M. al Sr. don Juan Miquel 11. Sureda y entregara a V.M. 122 Rs de pta 12. que con los 152 que han entregado son de 13. modo que con los 66 Rs del leynes 14. estan rematados los 327 Rs de pta 15. que tenia en mi poder de V.M.. Re 16. cividos destas partes, V.M. me lo 17. avise si esta satisfecho pa que yo 18. lo adbierta en mi libro. 19. Ya habra recivido por Barcelona 20. el Sr. Nuncio para el [^??^] 21. el mene de Jn Battistas pa lo del 22. Cauo de Artillero hedado en el Consejo 23. de Guerra qto ba para alli ynopor [^??^] [☞]";

function format(text, isModern) {
    let formattedText = text;
    if (!formattedText.includes('\n')) {
        if (!isModern) {
            formattedText = formattedText.replace(/(\b\d+\.\s)/g, '\n$1');
        } else {
            formattedText = formattedText.replace(/(Besa las manos|Guarde Dios|cuya vida guarde|Dios guarde|No se ofrece otra cosa|Por ahora no hay de nuevo|En este correo|Los días pasados|El señor d\.|El Sr\.|La carta dará|Ya habrá recibido|Aunque el Sr\.)/gi, '\n\n$1');
            formattedText = formattedText.replace(/^(Amigo y señor mío,|Muy señor mío,|Señor mío,)\s*/i, '$1\n\n');
        }
    }
...
console.log(format(text1, true));
console.log("--- MODERN 2 ---");
console.log(format(text2, true));
console.log("--- LITERAL 1 ---");
console.log(format(literal1, false));
